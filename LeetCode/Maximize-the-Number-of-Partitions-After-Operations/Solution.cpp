...
public:
    ...
    int solve(long long i, long long uniqueChars, bool canChange) {
        ...
        /*
                i << 27: Left-shifting i by 27 bits. This is done to occupy the
           most significant bits with the value of i. uniqueChars << 1:
           Left-shifting uniqueChars by 1 bit. This is done to make space for
           the least significant bit, which will be used to represent the
           boolean value of canChange. canChange: This boolean value is ORed
           with the result of the left-shift operations.
                                                                                            */
        ...
        if (canChange) {
            for (int newCharIndex = 0; newCharIndex < 26; ++newCharIndex) {
                ...
                if (newUniqueCharCount > K) {
                    result =
                        max(result, 1 + solve(i + 1, 1 << newCharIndex, false));
                } else {
